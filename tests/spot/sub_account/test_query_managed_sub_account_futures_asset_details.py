...
@mock_http_response(
    ...
    + encoded_string(expected_params),
    mock_item,
    200,
)
def test_query_managed_sub_account_futures_asset_details():
    """Tests the API endpoint to query managed sub-account futures asset details"""

    client = Client(key, secret)
    response = client.query_managed_sub_account_futures_asset_details(**send_params)
